...
{
    //tracks if textures were already loaded
    char boardTexturesPreloaded = logicBoardTexturesLoaded;
    ...
    char *filePathStr = AddPath("/sprites/logicBoardMenu.png");
    Texture2D menuTexture = LoadTexture(filePathStr);
    AddPathReuse("/sprites/boardBackground.png", &filePathStr);
    Texture2D backgroundTexture = LoadTexture(filePathStr);

    AddPathReuse(lbPath, &filePathStr);
    LogicBoard mainBoard = LoadLogicBoard(filePathStr);
    ...
    if(boardMenu.scale > 2.5f)boardMenu.scale = 2.5f;
    boardMenu.scale *= 7.5f;
    boardMenu.scale = floorf(boardMenu.scale);
    boardMenu.scale *= 0.2f;
    ...
    //the time no hint has been displayed onscreen
    char timeSinceHint = 26;
    ...
    while (!WindowShouldClose())
    {
        ...
        if(IsMouseButtonReleased(MOUSE_BUTTON_LEFT) && !justLoaded)
        {
        ...
        if(boardMenu.menuClicked)
        {
            ...
            if(IsMouseButtonReleased(MOUSE_BUTTON_LEFT))
            {
                ...
                )
                {
                    ...
                    StoreLogicBoard(filePathStr, mainBoard);
                }
                ...
                {
                    //frees allocated mem
                    if(!boardTexturesPreloaded)UnloadLogicBoardTextures();
        ...
        DisplayLogicBoard(mainBoard, worldCamera);
        ...
        //causes empty box to be drawn for a few frames when mouse not on menu
        //element, but still on menu
        if(menuHintNum == 0)
        {
            if(timeSinceHint < 26)
                timeSinceHint++;
        ...
        if((menuHintNum != 0) || (timeSinceHint < 25))
        {

            if(menuHintNum == 0)menuHintNum = prevMenuHint;

            unsigned char temp = (unsigned char) (
                ((float) (25 - timeSinceHint)) * (255.0f / 25.0f)
            );
            ...
            int xVals[] = {
                0.0f, boardMenu.scale, 2.0f * boardMenu.scale,
                (6.0f * boardMenu.scale) + hintSize.x,
                (7.0f * boardMenu.scale) + hintSize.x,
                (8.0f * boardMenu.scale) + hintSize.x
            };

            int yVals[] = {
                0.0f, boardMenu.scale, 2.0f * boardMenu.scale,
                (6.0f * boardMenu.scale) + hintSize.y,
                (7.0f * boardMenu.scale) + hintSize.y,
                (8.0f * boardMenu.scale) + hintSize.y
            };

            DrawRectangle(
                xVals[2], yVals[2], xVals[3] - xVals[2], yVals[3] - yVals[2],
                (Color) {0x78, 0x79, 0x7d, temp}
            );

            DrawRectangle(
                xVals[2], yVals[1], xVals[3] - xVals[2], yVals[2] - yVals[1],
                (Color) {0x1b, 0x1b, 0x1d, temp}
            );
            ...
            DrawText(
                ...
                7.0f * boardMenu.scale,
                IsMouseButtonDown(MOUSE_BUTTON_LEFT) ?
                (Color) {0xc6, 0xa1, 0x53, temp} :
                (Color) {0x4e, 0x40, 0x28, temp}
            );
    ...
    //frees string to current file being edited
    free(filePathStr);
